...
# you will have to change the code for 3D!

clist = [[ [] for col in range(n_cell)] for row in range(n_cell)] 

for i in range( 0, n_part ):
    
    clist[int(np.floor(posit[i,0]/l_cell))][int(np.floor(posit[i,1]/l_cell))].append(i)
...
for i in range( 0, n_cell ):
    ...
    for j in range( 0, n_cell ):
        ...
        # grab all particles in neighboring cells
        
        for k in range(-1, 2):
            
            for l in range(-1, 2):
                
                neig = clist[(i+k)%n_cell][(j+l)%n_cell]
                
                for m in range(len(cent)):
            
                    for n in range(len(neig)):
...
for index in range(len(pairlist)):
